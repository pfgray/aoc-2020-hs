...
import Text.Parsec.Char ( oneOf, spaces, digit, letter )
import Text.Parsec (Parsec)
import Data.Functor.Identity
import Text.ParserCombinators.Parsec

data Constraint = Constraint Int Int Char deriving Show

data PasswordEntry = PasswordEntry Constraint [Char]

instance Show PasswordEntry where
    show (PasswordEntry (Constraint minL maxL l) pw) = show minL ++ "-" ++ show maxL ++ " " ++ [l] ++ ": " ++ pw
...
parseLine :: GenParser Char st PasswordEntry
parseLine = do
    min <- many1 digit
    _   <- oneOf ['-']
    max <- many1 digit
    ...
    _   <- space
    pw  <- many1 letter
    return (PasswordEntry (Constraint (strToInt min) (strToInt max) l) pw)

countWhere :: (a -> Bool) -> [a] -> Int
countWhere f as = sum $ fmap (toInt . f) as
  where 
      toInt :: Bool -> Int
      toInt b = if b then 1 else 0

parseInput = traverse (parse parseLine "foo")

doIt = fmap (countWhere passwordIsValid2) (parseInput input)

doItToo = fmap (filter passwordIsValid2) (parseInput input2)

doItThree = (parseInput input2)

passwordIsValid :: PasswordEntry -> Bool
passwordIsValid (PasswordEntry (Constraint minL maxL l) pw) =
    let c = countWhere (== l) pw in
        c <= maxL && c >= minL
...
passwordIsValid2 :: PasswordEntry -> Bool
passwordIsValid2 (PasswordEntry (Constraint pos1 pos2 l) pw) =
    let atPos1 = at pos1 pw == Just l
        atPos2 = at pos2 pw == Just l
    in countWhere id [atPos1, atPos2] == 1
